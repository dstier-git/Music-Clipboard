...
def tick_to_measure_beat(tick, measures, division):
    ...
    for measure in measures:
        # Get measure length in ticks
        measure_length = 0
        time_sig_n, time_sig_d = get_time_signature(measure)
        
        if time_sig_n and time_sig_d:
            # Calculate measure length: (numerator * division * 4) / denominator
        ...
        if current_tick + measure_length > tick:
            ...
            beat = (beat_tick / division) + 1  # Beat starts at 1
            return measure_num, beat
        
        current_tick += measure_length
        measure_num += 1
    
    # Fallback: if we couldn't find the measure, return approximate values
...
def extract_pitches_with_position_from_mscx(mscx_file_path, output_file_path=None, debug=True, measure_range=None):
    ...
    try:
        ...
        # Get all measures for position calculation
        all_measures = list(root.iter('Measure'))
        if debug:
            print(f"Found {len(all_measures)} measures")
        
        # Create a list with measure numbers (measures are 1-indexed by position)
        ...
        # Filter measures if measure_range is specified
        if measure_range:
            start_measure, end_measure = measure_range
            # Filter to only the measures in range
            measures_with_numbers = [(no, m) for no, m in measures_with_numbers 
                                    if start_measure <= no <= end_measure]
            if debug:
                print(f"Filtering to measures {start_measure}-{end_measure}: {len(measures_with_numbers)} measures found")
        
        notes_with_position = []
        
        # Track current measure and tick position as we iterate
        current_tick = 0
        
        # If filtering, we need to calculate the starting tick position
        if measure_range:
            start_measure, end_measure = measure_range
            # Calculate tick position up to the start measure
            for idx, measure in enumerate(all_measures):
                # Get actual measure number
                measure_no_attr = measure.get('no')
                if measure_no_attr is not None:
                    try:
        ...
        # Iterate through filtered measures to track positions accurately
        for current_measure_num, measure in measures_with_numbers:
            ...
            if time_sig_n and time_sig_d:
                ...
                measure_length = division * 4  # Default 4/4
            
            # Track tick position within this measure
            measure_tick = 0
            has_chords = False
            
            # Look for Chord elements in this measure
            for chord in measure.iter('Chord'):
                has_chords = True
                # Get tick position - could be absolute or relative to measure
                chord_tick = None
                
                # Check if chord has tick attribute
                if 'tick' in chord.attrib:
                    chord_tick_val = int(chord.attrib['tick'])
                    # If tick is small, it's likely relative to measure start
                    ...
                    # Use current measure_tick as approximation
                    chord_tick = current_tick + measure_tick
                
                # Get duration to advance measure_tick
                ...
                for note in chord.findall('Note'):
                    ...
                    if pitch_elem is not None and pitch_elem.text:
                        ...
                        if debug and len(notes_with_position) <= 5:
                            print(f"Found note: {pitch_name} at {position_str} (tick: {chord_tick or (current_tick + measure_tick)})")
                
                # Advance measure_tick by duration
                if duration_elem is not None and duration_elem.text:
                    measure_tick += int(duration_elem.text)
                else:
                    measure_tick += division
            
            # Also check for direct Note elements (if no chords found in this measure)
            if not has_chords:
                for note in measure.iter('Note'):
                    ...
                    if pitch_elem is not None and pitch_elem.text:
                        ...
                        # Advance measure_tick (default quarter note)
                        measure_tick += division
            
            # Move to next measure
            current_tick += measure_length
        
        # Fallback: If no notes found, try simpler approach without position tracking
        ...
        # Final fallback: Try with namespaces
        if not notes_with_position:
        ...
        if notes_with_position:
            ...
            if debug:
                print(f"DEBUG: Saving to OUTPUT_DIR: {OUTPUT_DIR_NORMALIZED}")
            ...
            # Return both notes and the actual output path
